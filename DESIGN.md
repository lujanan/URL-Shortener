...
- **数据库**：MySQL
  - 使用 MySQL 8 作为持久化存储，满足常见生产场景需求，后期易于扩展与迁移。
  - 通过 Docker 中的独立 `mysql` 服务提供，数据通过 volume 挂载持久化。
- **缓存（可选扩展）**：Redis
  - 设计为可插拔层，初版可不依赖；在高并发或热点场景下加入。
- **配置管理**：环境变量（`PORT`、`DB_PATH`、`BASE_URL` 等）。
...
- **表：`short_links`**
  ...
  - 字段设计（与 MySQL 表结构一致）：
    ...
    - `expire_at DATETIME NULL`                         // 允许永不过期
    - `click_count BIGINT NOT NULL DEFAULT 0`           // 访问次数
    - `last_accessed_at DATETIME NULL`

  - **索引**
    - 唯一索引：`UNIQUE(code)`
    - 普通索引：`INDEX idx_expire_at (expire_at)`（定期清理可用）。

- **短码生成策略**

  - 使用 `id` 自增主键，生成后通过 Base62 编码得到短码。
  - Base62 字符集：`0-9A-Za-z`。
  - 避免碰撞：`code` 字段唯一约束，如遇冲突（极低概率）则重试或更换种子。
...
docker-compose.yml        <-- 编排 backend、frontend、mysql 三个服务
```
...
-- **容器设计**
  - **后端服务容器：`backend`**
    ...
    - 通过环境变量 `DB_DSN` 连接 MySQL，`BASE_URL` 用于拼接短链接域名。
  - **数据库容器：`mysql`**
    - 使用 `mysql:8.0.16` 镜像。
    - 通过环境变量配置数据库名、用户与密码（例如 `shorturl` / `shortuser` / `shortpass`）。
    - 使用 `mysql-data` volume 持久化 `/var/lib/mysql`。
  - **前端容器：`frontend`**
...
- **`docker-compose.yml` 关键点**
  - 服务 `backend`：
    - `build: ./backend`
    - `ports: ["8080:8080"]`
    - `environment`：`PORT`, `DB_DSN`, `BASE_URL` 等。
    - `depends_on`: `mysql`
  - 服务 `mysql`：
    - `image: mysql:8.0.16`
